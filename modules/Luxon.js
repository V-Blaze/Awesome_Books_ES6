...
class Formatter {
	...
	formatDateTimeFromString(dt, fmt) {
		const knownEnglish = this.loc.listingMode() === 'en',
			...
			meridiem = () =>
				knownEnglish ? meridiemForDateTime(dt) : string({ hour: 'numeric', hourCycle: 'h12' }, 'dayperiod'),
...
function fromStrings(weekdayStr, yearStr, monthStr, dayStr, hourStr, minuteStr, secondStr) {
	...
	if (weekdayStr) {
		result.weekday =
			weekdayStr.length > 3 ? weekdaysLong.indexOf(weekdayStr) + 1 : weekdaysShort.indexOf(weekdayStr) + 1;
	}
...
class DateTime {
    ...
    */
	resolvedLocaleOptions(opts = {}) {
		const { locale, numberingSystem, calendar } = Formatter.create(this.loc.clone(opts), opts).resolvedOptions(
			this
		);
